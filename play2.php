  ...
  <div id="sugoroku">
  		<div class="upperPart">
  			<div class="information">
					<p id="turn"><?= $game->name2 ?>のターン</p>
  				<p><input type="button"  id="rollBtn" value="さいころを振る" class="blue" onclick="rollDiceBlue()"></p>
					<p id="count"><?= $game->count2 ?>投目</p>
  				<p id="position">ゴールまで<?= 100 - $game->sum2 ?>マス</p>
  				<p id="goal"></p>
  		  </div>
  			<div id="result"></div>
  		</div>
	...
	<form class="none" action="updatep2.php" method="post">
		<input id="sum2" type="number" name="sum2">
		<input id="count2" type="number" name="count2">
		<input id="dice2" type="number" name="dice2">
		<input id="event" type="number" name="event">
		<input id="sb" type="submit" value="送信"　onclick="location.href='updatep2.php'">
	</form>
	<script type="text/javascript">
		var $id = function(id){ return document.getElementById(id); };
		var countVal2 = <?= $game->count2 ?>;　//投げた数をカウント
		var leftVal2 = <?= 100 - $game->sum2 ?>; //残りのマス
		var sum = <?= $game->sum1 ?>; //出目の合計
		var masuId = "m"+<?= $game->sum1 ?>;
		var sum2 = <?= $game->sum2 ?>;
		var masuId2 = "m"+<?= $game->sum2 ?>;
		var eventNum;
		$id(masuId).classList.add("red");
		$id(masuId2).classList.add("blue");
		$id("rollBtn").classList.add("blue");
	</script>
